...
export function BreathingTest() {
  ...
  useEffect(() => {
    ...
    if (isRecording) {
      interval = setInterval(() => {
        setBreathingScale((prev) => (prev === 1 ? 1.3 : 1));
      }, 2000);
    } else {
      setBreathingScale(1);
    }
  ...
  const handleContinue = () => {
    if (audioBlob) {
      ...
      reader.onloadend = () => {
        sessionStorage.setItem('breathingAudio', reader.result as string);
          ...
          <div className="space-y-6">
            <div className="bg-purple-50 rounded-2xl p-6">
              <p className="text-gray-700 leading-relaxed text-center">
                <span className="font-semibold">Take a deep breath and slowly exhale</span> for about 10 seconds.
              </p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <div className="relative mb-6">
                <div 
                  className={`w-40 h-40 rounded-full flex items-center justify-center transition-all duration-2000 ease-in-out ${
                    isRecording ? 'bg-purple-100' : isComplete ? 'bg-green-100' : 'bg-gray-100'
                  }`}
                  style={{ 
                    transform: isRecording ? `scale(${breathingScale})` : 'scale(1)',
                  }}
                >
                  {isRecording ? (
                    <div className="text-center">
                      <Circle className="w-16 h-16 text-purple-600 fill-purple-600 mb-2" />
                      <span className="text-3xl font-bold text-purple-600">{timeLeft}s</span>
                    </div>
                  ) : (
              ...
              <div className="text-center">
                {isRecording && (
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Breathe slowly and steadily</p>
                  </div>
                )}
                {isComplete && (
