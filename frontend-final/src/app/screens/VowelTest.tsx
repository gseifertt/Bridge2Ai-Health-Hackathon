...
export function VowelTest() {
  ...
  const handleContinue = () => {
    ...
    reader.onloadend = () => { sessionStorage.setItem('vowelAudio', reader.result as string); navigate('/test/breathing'); };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-white p-6">
      <div className="max-w-md mx-auto">
        <div className="mb-8">
          <button onClick={() => navigate('/test/cough')} className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 mb-4">
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <div className="w-full flex items-center justify-center gap-1 h-20 mb-4">
                {waveformBars.map((height, index) => (
                  <div key={index} className={`w-2 rounded-full transition-all ${isRecording ? 'bg-green-600' : 'bg-gray-300'}`} style={{ height: `${Math.max(height, 4)}%` }} />
                ))}
              </div>
              <div className="text-center">
                {isRecording && <p className="text-lg font-semibold text-gray-900">Recording... Say "Ahhh"</p>}
            ...
            <div className="space-y-3">
              {!isComplete && !isRecording && (
                <Button onClick={async () => { setTimeLeft(10); await startRecording(); }} className="w-full bg-gradient-to-r from-blue-500 to-green-500 hover:from-blue-600 hover:to-green-600 text-white h-14 rounded-full text-lg">
